    ...
    $.fn.zl_select = function (options) {
        ...
        objArray.each(function (index, ele) {
            ...
            container.on("click", function (e) {
                ...
                if(selectEle.attr("disabled")=="disabled")
                {
                    return;
                }
                e.stopPropagation();
                if (drop.is(":hidden")) {
                    ...
                    drop.show()
                        .position({
                            my: "left top+10",//减1，为了避免下边框双重重叠，所以向上移动1像素。
                            at: "left bottom",
                            of: $(this)
                        });
                    //内容区宽度
                    ...
                    $("body").on("click", function (e) {
                        ...
                        if ($(evt).closest("a.select-container").length == 0) {
                            drop.hide();
                            $(this).children(".select-container").removeClass("active");
                            $(".zl_select .select-container").removeClass("active");
                            $(".zl_select .select-container > div").removeClass("open");
        ...
        function _init_item(o) {
            ...
            var selectEle,optgroups,opts, copt, cv;
            prams.items.empty();
            selectEle = prams.selectEle;


            optgroups = selectEle.find("optgroup");
            //初始化时，select元素的当前值。
            ...
            if(optgroups.length!=0)
            {
                optgroups.each(function(){
                    ...
                    var group = $("<li />");
                    group.text($(this).attr("label"));
                    group.addClass("optgroups");
                    group.appendTo(prams.items);
